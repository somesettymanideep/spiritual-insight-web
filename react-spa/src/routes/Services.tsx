...
export default function Services() {
  useEffect(() => { setMeta("Astrology Services — Sri Durga Matha Astrology", "Explore our astrology services: black magic removal, love marriage, vashikaran, palm reading, tarot, and more."); }, []);
  return (
    <SiteLayout>
      ...
      <section className="py-20 bg-background">
        <div className="site-container grid lg:grid-cols-[1fr_320px] gap-10">
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {SERVICES.map((s, i) => (
              <article key={s.slug} className="group relative overflow-hidden rounded-3xl bg-card border border-border shadow-card hover:-translate-y-2 hover:shadow-elegant hover:border-primary/40 transition-all duration-500 mb-6">
                ...
                <div className="p-6">
                  <h3 className="font-display text-lg font-semibold mb-4 group-hover:text-primary transition-colors">{s.title}</h3>
                  <div className="flex items-center justify-start pt-4 border-t border-border/60">
                    <Link to={`/services/${s.slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary group-hover:gap-2.5 transition-all">
            ...
            <div className="rounded-2xl bg-card border border-border p-6 shadow-card">
              ...
              <ul className="space-y-2 text-sm">
                {SERVICES.slice(0, 8).map(s => (
                  <li key={s.slug} className="flex items-center gap-2 text-foreground/80">
